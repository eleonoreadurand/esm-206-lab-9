...
homes_lm1
#Note, thiss BAD because PricePerSqFt is calculated from the dependent variable Price --- biased overfitting of model results) 
...
class(homes_sub$Status) #check to see the class 
...
# c. Check levels:
levels(homes_sub$Status) #R tells us what levels it has

# d. Reassign reference level of "Status" to "Regular":
homes_sub$Status <-fct_relevel(homes_sub$Status, "Regular") #reassigns levels. this is reassigning the first level, but you could list all 3 in the order you want
...
#AIC is lower in lm1, so you would think to pick that, but we KNOW that lm1 is worse. Overall, heteroscedasticity for lm3 is OK, residuals normality definitely looks good, and conceptually and mathematically my model is making sense
#what are we sacrificing for a lower AIC? 
...
lm_table <- stargazer(homes_lm1, homes_lm3, type = "html") # can put as many models as you want
